...
│   ├── keyword_metrics.md     # 키워드 분석 메트릭 (검색량, 경쟁도 등)
...
| 0.5 | 키워드 분석 | (keyword-extractor) | (system) |
...
- **작업**:
  ...
  4. **키워드 분석 (추가)**:
     - input.txt를 바탕으로 구글 검색이나 AI 추천에 노출하기 쉬운 키워드를 분석
     - 각 키워드별 상세 메트릭 분석:
       - 구글 월간 검색량 (추정 수치)
       - 경쟁 콘텐츠 수 (추정 수치)
       - 키워드 난이도 (상/중/하)
       - 경쟁 강도 (높음/보통/낮음)
       - 성공 가능성 점수 (0-100)
     - 노출하기 쉬운 키워드(검색량 높고 경쟁도 낮은 키워드)를 추천
     - 제목 생성 시 추천 키워드 3개를 꼭 포함하도록 제한
  5. **키워드 메트릭 문서 생성**:
     - 분석된 키워드 메트릭을 `docs/keyword_metrics.md`에 저장
     - 포함 내용:
       - 요약 (추천 타겟 키워드 수, 필수 포함 키워드 수)
       - 제목에 필수 포함 키워드 리스트
       - 키워드 메트릭 표 (검색량, 경쟁 콘텐츠 수, 난이도, 경쟁 강도, 성공 점수)
       - 개별 키워드 상세 분석
       - 분석 기준 설명
  6. 후속 에이전트들에게 처리 완료 신호 전달
...
- **작업**:
  ...
  4. **키워드 성공 가능성 분석 (0-100점)**:
     - 제목과 본문에서 핵심 키워드 추출 (최대 5개)
     - 각 키워드별 구글 월간 검색량 조사
     - 각 키워드별 경쟁 콘텐츠 수 조사
     - 키워드 난이도(상/중/하) 평가
     - 경쟁 강도 분석 (높음/보통/낮음)
     - 성공 가능성 점수 산정 (0-100): 검색량 × (1/경쟁도) × 콘텐츠 품질점수의 가중 평균
     - 추천 타겟 키워드 선정 (성공 가능성 상위 3개)
  5. PASS/FAIL 판정 (총점 70점 이상 AND 제목 35점 이상 AND 본문 35점 이상 AND 키워드 성공 점수 50점 이상)
  6. FAIL 시 구체적인 피드백 및 키워드 최적화 제안 제공
- **API**: GLM-5 API 사용
- **출력 추가 항목**:
  - 키워드 성공 가능성 점수
  - 키워드 분석 상세 표 (검색량, 경쟁 콘텐츠 수, 난이도, 경쟁 강도, 성공 가능성)
  - 추천 타겟 키워드 리스트 (상위 3개)
...
┌─────────────────────────────────────────────────────────────────────┐
│                        사용자 입력                              │
│  [input.txt] 또는 [input_plan/CLAUDE.md]                      │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────────┐
│  에이전트 0: 입력 전처리 (blogger-doc-generator)                │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: input_plan/CLAUDE.md 또는 기존 input.txt         │   │
│  │ 처리: 웹서비스/블로그 글 판별 → input.txt 생성      │   │
│  │ 키워드 분석: GLM-5 API 호출 → 추천 키워드 추출    │   │
│  │ 출력: input.txt + keyword_analysis 딕셔너리            │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────────┐
│  키워드 메트릭 문서 저장                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: keyword_analysis (검색량, 경쟁도, 난이도 등)      │   │
│  │ 처리: 키워드 분석 보고서 생성                            │   │
│  │ 출력: docs/keyword_metrics.md                            │   │
│  │       (검색량, 경쟁 콘텐츠 수, 난이도, 경쟁 강도,       │   │
│  │        성공 점수 포함)                                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────────┐
│  에이전트 1: SEO 규칙 조사 (seo-rules-documenter)          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: 웹 검색 (2026 최신 SEO 트렌드)                │   │
│  │ 처리: SEO 규칙 수집 및 문서화                           │   │
│  │ 출력: docs/seo_rules.md                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                │
│  에이전트 2: AI 검색 규칙 조사 (ai-search-optimizer)         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: 웹 검색 (AI 검색 엔진 동작 원리)                │   │
│  │ 처리: AI 검색 노출 규칙 수집 및 문서화               │   │
│  │ 출력: docs/ai_search_rules.md                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────┬───────────────────────┬────────────────────┘
                    ↓                       ↓
        ┌─────────────────────────┐   ┌─────────────────────────┐
        │   docs/seo_rules.md   │   │docs/ai_search_rules.md│
        └─────────────────────────┘   └─────────────────────────┘
                    ↓                       ↓
                    └───────────┬───────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────────┐
│  에이전트 3: 검증 (seo-search-verifier)                    │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: docs/seo_rules.md, docs/ai_search_rules.md         │   │
│  │ 처리: 규칙 간 충돌, 최신성, 실현 가능성 검증    │   │
│  │ 출력: docs/validation_report.md                         │   │
│  │ 피드백: 문제 발견 시 에이전트 1, 2로 재작업 요청   │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────────┐
│  에이전트 4: 콘텐츠 생성 (blog-title-optimizer)            │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: input.txt, docs/seo_rules.md,                      │   │
│  │       docs/ai_search_rules.md, keyword_analysis             │   │
│  │ 처리: GLM-5 API 호출 → 제목 5개 + 한/영 본문 생성 │   │
│  │ 피드백 루프: 최대 3회 (에이전트 5의 피드백 반영)│   │
│  │ 출력: 생성된 콘텐츠 (5개 제목 + 본문 가이드)        │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 입력: 생성된 콘텐츠, docs/seo_rules.md,              │   │
│  │       docs/ai_search_rules.md                            │   │
│  │ 처리: GLM-5 API 호출 → 콘텐츠 품질 평가           │   │
│  │       - 제목 평가 (50점)                                │   │
│  │       - 본문 평가 (50점)                                │   │
│  │       - 키워드 성공 가능성 분석 (0-100점)                 │   │
│  │ PASS/FAIL 판정: (총점≥70 AND 제목≥35 AND 본문≥35    │   │
│  │                  AND 키워드≥50)                           │   │
│  │ 피드백: FAIL 시 개선 사항 반환 → 에이전트 4 전달     │   │
│  │ 출력: 평가 결과 (총점, 제목점수, 본문점수,           │   │
│  │       키워드 성공 점수, 피드백)                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                          ↓
...
│ 출력 데이터 구조:                                            │
│ {                                                            │
│   "input_txt": "생성된 input.txt 내용",                       │
│   "is_web_service": true/false,                               │
│   "keyword_analysis": {                                       │
│     "target_keywords": ["키워드1", "키워드2", ...],       │
│     "must_include_keywords": ["필수1", "필수2", "필수3"],│
│     "analysis": "요약 내용..."                               │
│   }                                                          │
│ }                                                            │
└─────────────────────────────┬───────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────────────┤
│ 입력 데이터:                                                  │
│ - input_txt (에이전트 0에서 생성)                             │
│ - docs/seo_rules.md (에이전트 1에서 생성)                     │
│ - docs/ai_search_rules.md (에이전트 2에서 생성)              │
│ - keyword_analysis (에이전트 0에서 생성)                        │
│                                                              │
│ 출력 데이터:                                                  │
│ - 5개 제목 (예상 클릭률, SEO 점수, 설명 포함)             │
│ - 한국어 본문 가이드 (H2/H3 구조, 키워드, FAQ 등)          │
│ - 영어 본문 가이드                                         │
└─────────────────────────────┬───────────────────────────────────┘
                              ↓ 콘텐츠 전달
┌─────────────────────────────────────────────────────────────────────┐
...
│   "title_score": 0-50,                                     │
│   "content_score": 0-50,                                    │
│   "keyword_success_score": 0-100,                             │
│   "keyword_analysis_detail": {                                  │
│     "keywords": [                                             │
│       {"name": "키워드1", "search_volume": 1000,           │
│        "competition_count": 50, "difficulty": "중",            │
│        "competition_level": "보통", "success_score": 75},       │
│       ...                                                     │
│     ],                                                        │
│     "recommended_keywords": ["키워드1", "키워드2", "키워드3"]│
│   },                                                          │
│   "feedback": "개선 사항..." (FAIL인 경우)                    │
│   "attempts": 1-3                                           │
│ }                                                            │
└─────────────────────────────┬───────────────────────────────────┘
                              ↓ PASS 시 / FAIL 시 피드백 전달
                    ┌─────────┴─────────┐
                    ↓                     ↓
            결과 저장 (output.txt)    에이전트 4 재시도 (최대 3회)
```
...
| 에이전트 | 입력 소스 | 출력 대상 | 의존 에이전트 | 후속 에이전트 |
|----------|------------|------------|----------------|---------------|
| 0: 입력 전처리 | input_plan/CLAUDE.md | input.txt | 없음 | 4 |
| 0.5: 키워드 분석 | input.txt | keyword_analysis, docs/keyword_metrics.md | 0 | 4 |
| 1: SEO 규칙 조사 | 웹 검색 (SEO 트렌드) | docs/seo_rules.md | 없음 | 3, 4, 5 |
| 2: AI 검색 규칙 조사 | 웹 검색 (AI 검색) | docs/ai_search_rules.md | 없음 | 3, 4, 5 |
| 3: 검증 | docs/seo_rules.md, docs/ai_search_rules.md | docs/validation_report.md | 1, 2 | 없음 (피드백만) |
| 4: 콘텐츠 생성 | input.txt, docs/seo_rules.md, docs/ai_search_rules.md, keyword_analysis | 콘텐츠 (5개 제목 + 본문 가이드) | 0, 0.5, 1, 2 | 5 |
| 5: 검색 랭킹 분석 | 콘텐츠, docs/seo_rules.md, docs/ai_search_rules.md | 평가 결과 + 피드백 | 1, 2 | 4 (FAIL 시) |
| 6: QA 및 테스트 | run.py, run.bat | 테스트 보고서 | 모든 코딩 작업 | 없음 |

### 데이터 전달 경로

```
input_plan/CLAUDE.md → [에이전트 0] → input.txt
                                            ↓
                                    keyword_analysis
                                            ↓
                                    docs/keyword_metrics.md
                                            ↓
docs/seo_rules.md ───┐                          │
                      ├──→ [에이전트 4] ───→ 콘텐츠 ──┬─→ [에이전트 5]
docs/ai_search_rules.md ─┘                          │
...
에이전트 0.5: 키워드 분석 → 키워드 추출 및 메트릭 분석
    ↓
키워드 메트릭 문서 저장 → docs/keyword_metrics.md (검색량, 경쟁도, 난이도, 성공 점수)
